      ...
      },
      {
        ...
        "fragments": [
          ...
          },
          {
            ...
            "invertInDarkMode": true,
            "description": "<strong>1) Labeling & Association</strong><br/>Feature: Progress ring labeling<br/>Scenario: Each progress ring has a meaningful label<br/>&nbsp;&nbsp;Given a progress ring is rendered on the page<br/>&nbsp;&nbsp;When a screen reader user navigates to the component<br/>&nbsp;&nbsp;Then the user hears a descriptive label that explains the purpose (e.g., 'Pronunciation')<br/>Scenario: Label is programmatically associated<br/>&nbsp;&nbsp;Given a progress ring with a visible or offscreen label element<br/>&nbsp;&nbsp;When the component is inspected<br/>&nbsp;&nbsp;Then the label is associated to the progress semantics via aria-labelledby or an equivalent mechanism<br/><br/><strong>2) Semantics (Native or ARIA)</strong><br/>Feature: Progress semantics exposed<br/>Scenario: Determinate progress using native semantics<br/>&nbsp;&nbsp;Given a progress ring representing a known value<br/>&nbsp;&nbsp;When the component is rendered with a native &lt;progress&gt; element<br/>&nbsp;&nbsp;Then the &lt;progress&gt; element has a value attribute and a max attribute<br/>&nbsp;&nbsp;And a screen reader announces the label and the current percentage or value<br/><br/><strong>3) Decorative SVG (No Duplicate Announcements)</strong><br/>Feature: Decorative SVG does not duplicate semantics<br/>Scenario: SVG is hidden from assistive technologies<br/>&nbsp;&nbsp;Given a progress ring uses an SVG for visual appearance<br/>&nbsp;&nbsp;When the component is inspected<br/>&nbsp;&nbsp;Then the SVG has aria-hidden='true' and focusable='false'<br/>&nbsp;&nbsp;And the accessible name and values come from the semantic element (&lt;progress&gt; or role='progressbar')<br/><br/><strong>4) Text Alternative / Visible Value</strong><br/>Feature: Visible text alternative<br/>Scenario: If a Visible percentage or summary is provided<br/>&nbsp;&nbsp;Given a determinate progress ring<br/>&nbsp;&nbsp;When the component is rendered<br/>&nbsp;&nbsp;Then if a visible text alternative such as '62%' or '62% complete' is displayed adjacent to the ring<br/>&nbsp;&nbsp;And this visible text is not redundantly announced (e.g., it has aria-hidden='true' if separate from the semantic element)<br/><br/><strong>5) Reduced Motion</strong><br/>Feature: Respects reduced motion preferences<br/>Scenario: Animations are disabled or simplified when user prefers reduced motion<br/>&nbsp;&nbsp;Given the user has enabled 'Reduce Motion' at the OS or browser level<br/>&nbsp;&nbsp;When the value of the progress ring changes there is no animation of the green ring<br/><br/><strong>6) Multiple Indicators (Uniqueness)</strong><br/>Feature: Multiple progress rings have unique labels<br/>Scenario: Unique labels for each ring in a list<br/>&nbsp;&nbsp;Given a page displays multiple progress rings at once<br/>&nbsp;&nbsp;When a screen reader user navigates through them<br/>&nbsp;&nbsp;Then each ring has a unique, descriptive label (e.g., 'File A upload', 'File B upload')<br/>&nbsp;&nbsp;And there is no ambiguity about which label belongs to which ring<br/><br/><strong>7) Non percentage values</strong><br/>Feature: Prevent fractions being converted to percentages for screen readers<br/>Scenario: A score is given as a fraction<br/>&nbsp;&nbsp;Given the max value is not 100<br/>&nbsp;&nbsp;When a screen reader user navigates to the component<br/>&nbsp;&nbsp;Then an assisted technology user hears the value as a fraction not a percentage<br/><em>Suggested way to achieve this</em>: If max is not 100 then add aria-valuetext 'value out of max'"
          }
        ]
      },
      {
