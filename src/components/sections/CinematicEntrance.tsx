...
function FogLayer({ delay, duration, direction }: { delay: number; duration: number; direction: 1 | -1 }) {
  return (
    <motion.div
      ...
      style={{
        background: `linear-gradient(${direction === 1 ? "90deg" : "270deg"}, transparent, rgba(61,155,155,0.04) 30%, rgba(61,155,155,0.08) 50%, rgba(61,155,155,0.04) 70%, transparent)`,
        filter: "blur(50px)",
      }}
      animate={{ x: [direction * -240, direction * 240] }}
...
const PARTICLES = [
  { x: "12%",  y: "18%", size: 2,   dur: 12, delay: 0   },
  { x: "28%",  y: "72%", size: 1.5, dur: 16, delay: 2.5 },
  { x: "44%",  y: "35%", size: 2.5, dur: 14, delay: 1   },
  { x: "58%",  y: "82%", size: 1.5, dur: 18, delay: 4   },
  { x: "70%",  y: "22%", size: 2,   dur: 11, delay: 1.5 },
  { x: "80%",  y: "55%", size: 3,   dur: 15, delay: 3   },
  { x: "88%",  y: "30%", size: 1.5, dur: 20, delay: 0.5 },
  { x: "18%",  y: "50%", size: 2,   dur: 13, delay: 5   },
  { x: "50%",  y: "15%", size: 1.5, dur: 17, delay: 2   },
  { x: "62%",  y: "65%", size: 2.5, dur: 12, delay: 3.5 },
  { x: "35%",  y: "88%", size: 2,   dur: 19, delay: 1   },
  { x: "92%",  y: "76%", size: 1.5, dur: 14, delay: 4.5 },
];
      ...
      {PARTICLES.map((p, i) => (
        <motion.div
          ...
          style={{
            ...
            boxShadow: `0 0 ${p.size * 3}px ${p.size}px rgba(61,155,155,0.35)`,
          }}
          animate={{
            y: [-10, -28, -10],
...
function NameSVG() {
  return (
    <div className="relative overflow-hidden">
      ...
      {/* One-shot shimmer sweep across the name */}
      <motion.div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: "linear-gradient(105deg, transparent 30%, rgba(145,251,255,0.28) 50%, transparent 70%)",
          backgroundSize: "200% 100%",
        }}
        initial={{ backgroundPositionX: "-100%" }}
        animate={{ backgroundPositionX: "200%" }}
        transition={{ duration: 1.1, delay: 1.2, ease: [0.4, 0, 0.2, 1] }}
      />
    </div>
  );
}
...
export default function CinematicEntrance() {
  ...
  const pTx = useMotionValue(0); const sTx = useSpring(pTx, { stiffness: 90, damping: 30 });
  const pTy = useMotionValue(0); const sTy = useSpring(pTy, { stiffness: 90, damping: 30 });
  const pPx = useMotionValue(0); const sPx = useSpring(pPx, { stiffness: 60, damping: 24 });
  const pPy = useMotionValue(0); const sPy = useSpring(pPy, { stiffness: 60, damping: 24 });
  ...
  useEffect(() => {
    pTx.set(mousePos.x * 16);  pTy.set(mousePos.y * 10);
    pPx.set(mousePos.x * -26); pPy.set(mousePos.y * -16);
  ...
  return (
    <div ref={containerRef} className="font-montserrat relative min-h-screen overflow-hidden" style={{ background: "var(--bg-primary)" }}>
      ...
      <FogLayer delay={5}  duration={26} direction={-1} />
      <FogLayer delay={11} duration={18} direction={1}  />
      ...
      <motion.div
        ...
        animate={{
          left: `${50 + mousePos.x * 22}%`,
          top:  `${50 + mousePos.y * 22}%`,
        }}
        transition={{ type: "spring", stiffness: 45, damping: 28 }}
        style={{
          width: 700, height: 700,
          transform: "translate(-50%, -50%)",
          background: "radial-gradient(circle, rgba(61,155,155,0.09) 0%, transparent 70%)",
          filter: "blur(70px)",
        }}
      />
      ...
      <motion.div
        ...
        style={{ height: 1, background: "linear-gradient(90deg, transparent, rgba(145,251,255,0.12), transparent)" }}
        animate={{ top: ["0%", "100%"] }}
      ...
      <AnimatePresence>
        {phase < 3 && (
          ...
          >
            {/* Expanding rings — two staggered pulses */}
            {[0, 0.4, 0.8].map((d, i) => (
              <motion.div
                key={i}
                className="absolute rounded-full border border-[#3D9B9B]/20"
                initial={{ width: 0, height: 0, opacity: 0.8 }}
                animate={{ width: 420 + i * 80, height: 420 + i * 80, opacity: 0 }}
                transition={{ duration: 2.2, ease: "easeOut", delay: d }}
              />
            ))}

            {/* Crosshair center */}
            <div className="relative flex items-center justify-center w-16 h-16">
              <motion.div className="absolute w-8 h-px bg-[#3D9B9B]/60"
                initial={{ scaleX: 0 }} animate={{ scaleX: 1 }}
                transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
              />
              <motion.div className="absolute h-8 w-px bg-[#3D9B9B]/60"
                initial={{ scaleY: 0 }} animate={{ scaleY: 1 }}
                transition={{ duration: 0.6, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
              />
              <motion.div
                className="w-2 h-2 rounded-full bg-[#3D9B9B]"
            ...
            {/* Encrypted name */}
            <motion.p
              className="mt-10 text-[11px] uppercase tracking-[10px] font-light"
              style={{ color: "var(--fg-20)" }}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ delay: 0.4, duration: 0.8 }}
            >
              <EncryptedReveal text="AHMED SOUDAN" delay={400} />
            </motion.p>

            {/* Progress bar fill */}
            <div className="absolute bottom-[28%] left-1/2 -translate-x-1/2 w-[200px] h-px overflow-hidden">
              <div className="w-full h-full" style={{ background: "rgba(61,155,155,0.12)" }} />
              <motion.div
                className="absolute top-0 left-0 h-full"
                style={{ background: "linear-gradient(90deg, transparent, rgba(61,155,155,0.6), rgba(145,251,255,0.8))" }}
                initial={{ width: "0%" }}
                animate={{ width: "100%" }}
                transition={{ duration: 3.4, ease: [0.4, 0, 0.6, 1] }}
              />
            </div>
          </motion.div>
        )}
        ...
        <div className="relative z-30 flex min-h-screen items-center">
          ...
          <div className="absolute inset-0 overflow-hidden">
            ...
            <div className="absolute inset-0 flex items-center justify-end">
              <motion.div
                className="w-[520px] h-[520px] lg:w-[760px] lg:h-[760px] rounded-full"
                style={{
                  background: "radial-gradient(circle, rgba(61,155,155,0.16) 0%, rgba(61,155,155,0.06) 35%, transparent 70%)",
                  filter: "blur(90px)",
                  marginRight: "4%",
                }}
            ...
            {/* Photo — cinematic rising reveal + parallax */}
            <motion.div
            ...
            >
              <div className="relative w-[100vw] h-[90vh] max-w-[550px] lg:w-[50vw] lg:max-w-[700px] lg:h-[100vh] lg:mr-[5%] translate-y-[5%]">
                <Image
                  ...
                  alt="Ahmed Soudan"
                  fill
                  className="object-contain object-bottom drop-shadow-[0_0_80px_rgba(61,155,155,0.12)]"
                  priority
                />
              </div>
            </motion.div>

            {/* Blending overlays — unchanged */}
            <div className="absolute inset-0 pointer-events-none"
              style={{ background: "radial-gradient(ellipse 60% 55% at 55% 38%, transparent 35%, rgba(var(--vignette-rgb),0.5) 60%, var(--bg-primary) 85%)" }}
            />
            <div className="absolute bottom-0 left-0 right-0 h-[50%] pointer-events-none"
              style={{ background: "linear-gradient(to top, var(--bg-primary) 0%, var(--bg-primary) 20%, rgba(var(--vignette-rgb),0.95) 40%, rgba(var(--vignette-rgb),0.6) 60%, transparent 100%)" }}
            />
            <div className="absolute inset-0 pointer-events-none lg:hidden"
              style={{ background: "linear-gradient(to right, rgba(var(--vignette-rgb),0.85) 0%, rgba(var(--vignette-rgb),0.5) 30%, transparent 60%)" }}
            />
            <div className="absolute inset-0 pointer-events-none hidden lg:block"
              style={{ background: "linear-gradient(to right, var(--bg-primary) 0%, rgba(var(--vignette-rgb),0.95) 20%, rgba(var(--vignette-rgb),0.4) 35%, transparent 50%)" }}
            />
            <div className="absolute inset-0 pointer-events-none"
              style={{ background: "linear-gradient(to left, var(--bg-primary) 0%, rgba(var(--vignette-rgb),0.7) 8%, transparent 20%)" }}
            />
            <div className="absolute top-0 left-0 right-0 h-[18%] pointer-events-none"
              style={{ background: "linear-gradient(to bottom, var(--bg-primary) 0%, rgba(var(--vignette-rgb),0.6) 50%, transparent 100%)" }}
            />
          </div>
            ...
            >
              ...
              <motion.div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-4 mt-6" variants={slideUp}>
                <p className="text-[11px] font-semibold uppercase tracking-[5px]" style={{ color: "var(--accent)" }}>
                  Product Designer <span style={{ color: "var(--fg-30)" }}> · </span> <YearCounter target={20} delay={3600} /> yrs
                </p>
                <motion.div
                  className="h-px"
              ...
              <motion.div className="mt-8 lg:mt-8 max-w-md relative" variants={fadeIn}>
                <div className="border-l-2 pl-5" style={{ borderColor: "var(--accent)", opacity: 0.5 }}>
                  <p className="text-[15px] leading-[1.65]" style={{ color: "var(--fg-60)" }}>
                    <span style={{ color: "var(--accent)", textShadow: "0 0 20px rgba(var(--accent-rgb), 0.35)" }}>20 years</span> crafting digital experiences — from architecture to product design.{" "}
                    <span style={{ color: "var(--accent)", textShadow: "0 0 20px rgba(var(--accent-rgb), 0.35)" }}>300+ projects</span> delivered across the globe. Currently shaping the future of education at{" "}
                    <span style={{ color: "var(--accent)", textShadow: "0 0 20px rgba(var(--accent-rgb), 0.35)" }}>Oxford University Press</span>.
                  </p>
                </div>
              </motion.div>
      ...
      {phase === 3 && (
        ...
        >
          ...
          />
          <p className="text-[10px] uppercase tracking-[4px]" style={{ color: "var(--fg-15)" }}>Scroll</p>
        </motion.div>
      )}
    </div>
  );
