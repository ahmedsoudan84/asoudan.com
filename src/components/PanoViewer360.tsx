...
export default function PanoViewer360({
  ...
  return (
    ...
    >
      ...
      >
        {[0, 1, 2].map((i) => (
          <div key={i} className="relative h-full" style={{ width: '33.333%', flexShrink: 0 }}>
            ...
            />
            {/* Markers inside the strip copy */}
            {isClient && markers.map((marker) => (
              <div
                key={marker.id}
                className="absolute pointer-events-none"
                style={{
                  left: `${((marker.longitude % 360 + 360) % 360) / 3.6}%`,
                  top: `${50 - marker.latitude}%`,
